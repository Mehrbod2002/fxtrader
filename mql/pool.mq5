...
void ProcessTradeRequest(string json_data) {
   ...
   if (!json.hasValue("trade_id")) { // Check if parsing failed or trade_id is missing
       Print("Failed to parse JSON or missing trade_id: ", json_data);
       delete json;
       return;
   }
...
bool CanMatchTrades(PoolTrade &trade1, PoolTrade &trade2) {
   ...
   if (trade1.order_type == "BUY_STOP" && market_price >= trade1.entry_price) return true;
   if (trade1.order_type == "SELL_STOP" && market_price <= trade1.entry_price) return true;
   if (trade2.order_type == "BUY_STOP" && market_price >= trade2.entry_price) return true;
   if (trade2.order_type == "SELL_STOP" && market_price <= trade2.entry_price) return true;
...
bool SendTradeResponse(string trade_id, string user_id, string status, string matched_trade_id) {
   ...
   char data[], result_data[];
   StringToCharArray(json_str, data, 0, StringLen(json_str));
   string headers = "Content-Type: application/json\r\n";

   int res = WebRequest("POST", api_url, headers, 5000, data, result_data, headers);
   bool success = (res == 200);
   
   if (success) {
      Print("Sent response: ", json_str);
   } else if (res != -1) {
      Print("HTTP error sending response: ", res);
   } else {
...
string FetchTradeRequest() {
   string result = "";
   char data[], result_data[];
   string headers = "Content-Type: application/json\r\n";
   int res = WebRequest("GET", api_url, headers, 5000, data, result_data, headers);
   
   if (res == 200) {
      result = CharArrayToString(result_data);
      Print("Received trade request: ", result);
   } else if (res != -1) {
